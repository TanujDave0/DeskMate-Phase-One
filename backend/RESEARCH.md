# FUNBOT — Similar Projects Research

Research survey of open-source projects relevant to FUNBOT's architecture.
...
- Architecturally useful for FUNBOT: The system prompt is built as
  `prefix + persona_string + suffix`, configured in `config.json`. Runtime
  commands `/persona`, `/stock`, `/reset`, `/custom` swap personalities
  without restart. This is almost exactly the shape FUNBOT needs — we'd
...
- Pitfall/gotcha: No persona routing logic, no mixing/interpolation between
  ...
  brief"). FUNBOT's fixed 5 anchors avoids the drift, but we still need
...
- Stack: C#, Ollama.
- Architecturally useful for FUNBOT: Demonstrates a *multi-axis* tone
  system (tone × politeness × expertise) rather than a single dial.
  FUNBOT's single sass dial is simpler and better — but this project is
...
- Pitfall/gotcha: Trait combinations can produce incoherent personas
  ("extremely sarcastic + extremely polite"). FUNBOT's 5 fixed named
...
- Architecturally useful for FUNBOT: The **Character Card** format
...
- Pitfall/gotcha: Massive scope creep territory. SillyTavern has world
  ...
  macros. For FUNBOT personal use, pick the 20% that matters
...
- Architecturally useful for FUNBOT: Not the Big Five approach (wrong
  ...
  FUNBOT: `sassLevel`) into a system prompt string. If FUNBOT ever
...
- Stack: Python (66%), JS/HTML/CSS for the server UI, multiple LLM
  backends.
- Architecturally useful for FUNBOT: **Configurable persistent humor and
  honesty levels** — the closest prior art to FUNBOT's sass dial in the
  ...
  path FUNBOT needs later. Building now against a clean HTTP contract
...
- Pitfall/gotcha: Gemini Live + ElevenLabs = recurring cloud costs and
  content-policy exposure (will refuse at sass=100). FUNBOT's Ollama-first
...
- Architecturally useful for FUNBOT: Proof that an offline, laptop-class
  small LLM (Phi-4-mini ~ 3.8B) is viable for a TARS-style assistant
  without a GPU. Relevant given FUNBOT's laptop + integrated-graphics
...
- Architecturally useful for FUNBOT: The anti-pattern writeup —
  personality here is *only* the voice (deep/slow pyttsx3 config), not the
  text. Responses are vanilla ChatGPT. This is what FUNBOT must *not* do:
...
- Pitfall/gotcha: README handwaves "you can add iconic lines for humor
  ...
  (character consistency) to "optional enhancement." FUNBOT has to do that
...
- Stack: Parakeet TDT STT, Kokoro / custom GLaDOS TTS, Silero VAD,
  ...
  FastVLM vision.
- Architecturally useful for FUNBOT: Multi-component context-merging
  pattern — the system prompt is assembled each turn from
  `personality core + task slots (weather/news/vision) + emotional state
  (PAD model) + long-term memory`. Generalizes well: FUNBOT's per-turn
...
- Pitfall/gotcha: HEXACO + PAD emotional modelling is wildly overkill
  for FUNBOT's 5 fixed anchors. Steal the "context-merging" idea, not the
...
- Architecturally useful for FUNBOT: Shows how to wrap Kokoro behind a
  FastAPI wrapper for TTS — useful pattern if FUNBOT later replaces
...
- Pitfall/gotcha: Documents a real problem — **audio feedback loops**
  when the mic picks up the TTS output. Browser-native STT on FUNBOT
...
- Architecturally useful for FUNBOT: **This is the closest reference
  repo to FUNBOT's backend shape.** Clean separation of streaming vs.
  non-streaming endpoints, Pydantic request models. Current FUNBOT
...
- Architecturally useful for FUNBOT: Illustrates where the complexity
...
- Pitfall/gotcha: Scope inflation. Session storage, multi-user, OpenAI
  compat — none of this is on FUNBOT's roadmap. Watch out for dragging
...
- Architecturally useful for FUNBOT: Canonical reference for the "no
  cloud, no subscription" voice-assistant loop on a CPU. Relevant when
  FUNBOT eventually replaces browser-STT with a self-hosted alternative.
...
- Architecturally useful for FUNBOT: Expression tags — responses
  include inline `<giggle>`, `<sigh>`, `<laugh>` etc. that the TTS
  interprets. If FUNBOT ever wants sass-level-dependent vocal delivery
...
- Architecturally useful for FUNBOT: This is *the* reference for the
  voice -> physical-robot arc. The SDK intentionally separates
  "behaviors" (head poses, antenna animations, greetings) from the
  brain. A `POST /api/chat` backend like FUNBOT's plugs in directly —
  ...
  FUNBOT already uses — good signal).
...
- Architecturally useful for FUNBOT: Direct evidence that small Ollama
  models (Gemma 2 2B) + quantized STT fit on constrained hardware.
  Validates FUNBOT's integrated-graphics laptop path using similar sized
  models (llama3.2:3b, qwen2.5:3b, phi3:mini).
- Pitfall/gotcha: 3.5-7.5 s end-to-end is *noticeable lag*. Below
  GLaDOS's 600 ms target by an order of magnitude. For FUNBOT, this is
...
- Architecturally useful for FUNBOT: Physical TARS hardware reference
  for when the voice-to-robot migration happens. The aiortc server runs
  independently of the LLM brain — clean separation. A FUNBOT robot
...
- Stack: Available as Ollama models: `dolphin-mistral`, `dolphin-llama3`,
  ...
  model — pull and go.
- Architecturally useful for FUNBOT: The canonical answer to
  ...
  source of tone/policy, not baked-in RLHF. FUNBOT's sass-100 "UNHINGED"
...
- Architecturally useful for FUNBOT: Strong at *staying in character*
...
- Pitfall/gotcha: The SillyTavern community leans heavily
  NSFW/role-play. FUNBOT's goal is comedy, not erotica — filter their
...
1. **`prefix + sass_anchor + suffix` system-prompt template**
   ...
   ("You are FUNBOT, a helpful assistant..."), the per-anchor body sets
...
6. **Plan for latency, not real-time**
   ...
   requires a GPU or cloud API, both outside FUNBOT's constraints.
...
3. **Don't pull in LangGraph / LangChain / Redis / Postgres too early.**
   ...
   FUNBOT's current contract (one endpoint, no history persistence, one
